...
4. Copy the `.env.example` file to `.env` in the root directory.
...
2. Update environment variables in the `.env` file.
    ...
    + `TRAEFIK_ACME_EMAIL`: The email address for the ACME challenge.
    + `TRAEFIK_ACME_DNS_PROVIDER`: The DNS provider for the ACME challenge. Set the value to `cloudflare`.
...
You can change the log configuration by updating the `.env` file.
